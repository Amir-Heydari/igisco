...
export default async function HomeBlog({ lang }) {

    const response = await axios.get('http://185.97.117.44:1337/api/articles/');
                                    ...
                                    return (
                                        <li key={v.id}>
                                            <div className="item">
                                                ...
                                                <div className="title_holder">
                                                    <h3><Link href={`/blogs/${v.id}`}>{lang === "fa" ? v.title_pr : v.title_en}</Link></h3>
                                                    <p className="t_footer"><Link href={`/blogs/${v.id}`}>{Home.News.readmore}</Link></p>
                                                </div>
                                            </div>
                                        </li>
                                    )
